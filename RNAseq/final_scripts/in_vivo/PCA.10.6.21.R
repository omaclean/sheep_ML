...
plotlypdf=plot_ly(x=PCA$x[!is.na(cond_i),2],y=PCA$x[!is.na(cond_i),3],
        color=factor(cond_i[!is.na(cond_i)],levels=conditions),colors=pal,alpha=0.85,
        text=~paste(sapply(colnames(counts2),function(x) strsplit(x,'_')[[1]][1]),'dpi=',dpis,sex)[!is.na(cond_i)])%>%
  add_markers(marker=list(size=12))%>%
...
pdf("/home/oscar/scripts/github/sheep_ML/outdir/RNA_seq/invivo_PCA_2_3.pdf",width=10,height=10)
par(mar=c(5,5,5,1))
  plot(x=PCA$x[!is.na(cond_i),2],y=PCA$x[!is.na(cond_i),3],
          col=scales::alpha(pal[as.numeric(factor(cond_i[!is.na(cond_i)],levels=conditions))],0.75),
          xlab=paste("PCA2 (var prop=",round(100*PCA$sdev[2]^2/sum(PCA$sdev^2),1),'%)'),
          ylab=paste("PCA3 (var prop=",round(100*PCA$sdev[3]^2/sum(PCA$sdev^2),1),'%)'),pch=19,
          cex=1.8,
          cex.axis=1.4,cex.lab=1.9,main='PCA of in vivo RNAseq data',cex.main=2.5)
      legend("topleft",legend=conditions,col=pal,pch=19,bty="n",cex=1.5)
dev.off()
