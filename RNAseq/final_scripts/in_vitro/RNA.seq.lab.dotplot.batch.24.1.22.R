library(scales);library(RColorBrewer);library(ggplot2);library(gridExtra);library(edgeR)
library(venn);library(sva)
...
batches=sapply(colnames(counts_in),function(x) paste(strsplit(x,'_')[[1]][2],
                                                     collapse='_'))
...
#order columns
counts2=counts[,c(grep('Mock',colnames(counts)),grep(inf_states[1],colnames(counts)),
...
plot(apply(counts2,2,function(x) var(x/sum(x)))[c(grep('Mock',colnames(counts2)),grep(inf_states[1],colnames(counts2)),
...
batches=sapply(colnames(counts2),function(x) paste(strsplit(x,'_')[[1]][2],
                                                   collapse='_'))
...
for(inf in inf_states){
  ...
  write.csv(TT_save,paste('~/Documents/sheep_megadata/RNA.seq.lab.nov.21/GLM.results.merge.tab.all.batch.combatseq/',inf,'.csv',sep=''))
    print(c(inf,length(which(TT$table$FDR<0.05)),
      ...
      plot_dat=rbind(plot_dat,as.matrix(
        cbind(TT$table$logFC,TT$table$FDR<0.05,
              rep(paste(inf,'.',sep=''),nrow(TT)),
              rep(paste(inf,'.','\n(# sig&logFC>2=',
                        length(which(TT$table$FDR<0.05 &abs(TT$table$logFC)>2)),')',sep=''),
                  nrow(TT)),
              rownames(TT)
...
for(time in c('6h','12h')){
  plot_i=plot_i+1
    ...
    plots[[plot_i]]= ggplot(plot_dat,aes(x=condition_lab,y=logFC,color=colour))+
      ...
      geom_jitter()+scale_colour_manual(values=colpal[sort(unique(as.numeric(plot_dat$colour)))])+
      theme_bw()+ylim(-3,8)+
      xlab('virus:location')+
    ...
    pres_abs=matrix(ncol=length(conds),
    ...
    colnames(pres_abs)=paste(conds,'\nN=',sapply(1:length(conds),function(x)
      sum(pres_abs[,x])),sep='')
    venns[[time]]=venn((as.data.frame((pres_abs),row.names = NULL,
                                      col.names=NULL)),zcolor="style",ggplot=T)+
    ...
    write.csv(pres_abs2,file=paste('~/Documents/sheep_megadata/RNA.seq.lab.nov.21/venn.ISGs/combat_batch.all.tab',
                                   time,'.csv',sep=''))
}
...
plot(dat6$logFC,dat12$logFC,col=col2,pch=19,main=paste(inf_plot,'one combined 6+12 hr table'),
...
legend(x='bottomright',col=1:4,legend=paste(
  ...
  c(length(which(col==1)),length(which(col==2)),
    length(which(col==3)),length(which(col==4)))),pch=19,bty='n')
...
col[dat6$FDR<0.05&dat12$FDR<0.05]=4
col2=alpha(col,(col/4))
plot(dat6$logFC,dat12$logFC,col=col2,pch=19,main=paste(inf_plot,'two tables'),
     xlab='6hrs logFC',ylab='12hrs logFC')
legend(x='bottomright',col=1:4,legend=paste(
  c('DE neither ;N=','DE only at 12hr; N=','DE only at 6hr;N=','DE both ; N='),
    c(length(which(col==1)),length(which(col==2)),
