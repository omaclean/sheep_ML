...
for(dat_i in 2:(length(types_loop1)-1)){
   for(dat_j in (dat_i+1):length(types_loop1)){
    ...
    types_list1=c(types_list1[1],types_list1[dat_i],types_list1[dat_j])
      for(lev2 in c('1','3','7')){
        dpi_levels[2]=lev2
        dat1=read.csv(file=paste('~/Documents/sheep_megadata/RNA_Seq_1.12.20/mixed_effects/3.tier.GLM/',
    ...
    print(lev2)
      ...
      #   for(j in (i+1):3){
          ...
          abline(0,1,lty=2)
          legend(col=colpal,legend=legend_set,pch=19,x="bottomright",bty="n")
