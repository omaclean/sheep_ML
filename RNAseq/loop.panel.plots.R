...
for(lev2 in c('1','3',7)){
  ...
  for(i in 1:length(types_loop1)){
    ...
    #   types_list1=types_list1_loop[i]
    #   types_list2=types_list2_loop[i]
    png(filename=paste('~/Pictures/plots/Sheep_megadata/RNA_seq/init_analyses/big_loop/',
                       types_list1,'.and.',types_list2,'.dpi.',dpi_levels[1],'.vs.',
                       dpi_levels[2],'.png',sep=''))
    par(mfrow=c(2,2))
    ...
    dev.off()
    names_plot=gene_names$gene_name[match(genes_hits[[1]],gene_names$sheep_ID)]
    mutual_DE=which(genes_hits[[2]]<0.05&genes_hits[[5]]<0.05)
    dat_write=cbind(names_plot,genes_hits[[1]],genes_hits[[2]],
                    genes_hits[[3]],genes_hits[[5]],genes_hits[[6]])
    ...
    write.csv(dat_write[mutual_DE,],
              file=paste('~/Pictures/plots/Sheep_megadata/RNA_seq/init_analyses/big_loop/either.DE.',
                         location1,'.',types_list1,'.and.',location2,'.',types_list2,'.dpi.',dpi_levels[1],'.vs.',
                       dpi_levels[2],'.csv',sep=''),row.names = F)
  }
}
# 
