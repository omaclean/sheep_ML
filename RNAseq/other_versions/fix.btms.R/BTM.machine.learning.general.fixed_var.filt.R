...
if(!exists('run')){
  ...
  overlap_prop=rep(NA,nrow(mat3)^2)
  counter=0
  hits=matrix(ncol=3,nrow=0)
  for(i in 1:(ncol(dat)-1)){
    for(j in (i+1):ncol(dat)){
      counter=counter+1
      overlap_prop[counter]=length(intersect(
        dat[!is.na(dat[1:nrow(dat),colnames(dat)[i]]),colnames(dat)[i]],
        dat[!is.na(dat[1:nrow(dat),colnames(dat)[j]]),colnames(dat)[j]]))/
        max(c(length(which(!is.na(dat[2:nrow(dat),colnames(dat)[i]]))),length(which(!is.na(dat[2:nrow(dat),colnames(dat)[j]])))))
      if(overlap_prop[counter]>0.5){
        hits=rbind(hits,c(colnames(dat)[c(i,j)],round(overlap_prop[counter],3)))
      }
    }
  }
  length(unique(c(hits[,1],hits[,2])))
  overlap_prop=overlap_prop[!is.na(overlap_prop)]
  hist(overlap_prop[overlap_prop>0.1])
  length(which(overlap_prop>0.3))
  length(which(overlap_prop>0.5))
  length(which(overlap_prop>0.1))
  ####
...
png(filename=paste('~/Pictures/plots/Sheep_megadata/RNA_seq/init_analyses/BTM.mach.learn.new/'
                   ,cond,'_dpi',dpi_plot,'.var.filt.png',sep=''),width=1100,height=200*length(plots))
